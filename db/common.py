...
def export(src: str, dst: str, *, encoding: str='shift-jis', remove_keys: list[str]=['list_idx', 'each_idx', 'created_at']) -> None:  # HACK: 240303 単体テスト可能な形で実装せよ。 (改行 / None の件はテストコードが必要と思う。)
    """
    ...
    try:
        ...
        with open(dst, 'w', newline='', encoding=encoding) as f:
            ...
            writer.writerow([key for key in keys if key not in remove_keys])
            each_url_table_iter = session.query(EachUrlTable).order_by(EachUrlTable.list_idx, EachUrlTable.each_idx)

            for each_url_table in each_url_table_iter:  # INFO: 240306 each_url_table は、１行のレコードのデータクラス。
                oneline = []
                for key in keys:
                    if key not in remove_keys:
                        if getattr(each_url_table, key) is None:
                            oneline.append('')
                        else:
                            oneline.append(str(getattr(each_url_table, key)))
